...
or a manual PM2 + Nginx setup ([appendix](#appendix-manual-deployment-without-docker)), in front of either
a local MariaDB container or a separate Amazon RDS instance. Either way, a thin host-level Nginx handles
TLS termination and is the only thing exposed to the internet.

```mermaid
flowchart LR
    Client((Browser)) -->|HTTPS 443| HostNginx["Host Nginx (TLS termination)"]
    HostNginx -->|"127.0.0.1:8080"| FE["frontend container (Nginx + built dist/)"]
...
5. **Security group** — inbound rules:
   ...
   Leave 5000 (backend) and any Vite ports closed to the internet entirely — Nginx is the only public
   entry point.
...
backend API ([backend/Dockerfile](backend/Dockerfile)), and the frontend, served by an internal Nginx
from a production build ([frontend/Dockerfile](frontend/Dockerfile), [frontend/nginx.conf](frontend/nginx.conf))
— as one stack.
...
`127.0.0.1:5000`), and `frontend` (Nginx serving the built app on `127.0.0.1:8080`) — the backend waits
for the database's healthcheck before it starts, via `depends_on: condition: service_healthy`.
...
## 6. Put Nginx + TLS in front of the Docker stack

The frontend container already proxies `/api/*` to the backend internally, so the host Nginx just needs
to forward everything to `127.0.0.1:8080` and handle TLS.

Create `/etc/nginx/sites-available/planetone`:
...
```bash
sudo ln -s /etc/nginx/sites-available/planetone /etc/nginx/sites-enabled/
sudo rm -f /etc/nginx/sites-enabled/default
sudo nginx -t && sudo systemctl reload nginx

# TLS via Let's Encrypt — edits the config above to add the `listen 443 ssl` block + auto-renewal
...
curl https://your-domain.com/api/health # from your own machine, through host Nginx + TLS
```
...
run the backend directly with PM2 and serve the frontend build as static files from Nginx.
...
cd frontend && npm run build && cd ..   # produces frontend/dist/, served by Nginx below
```

### A.5 Configure Nginx to serve `frontend/dist` and proxy `/api`
...
server {
    ...
    root /opt/planetone/frontend/dist;
    index index.html;
...
Enable it and add TLS the same way as [§6](#6-put-nginx--tls-in-front-of-the-docker-stack) above.
